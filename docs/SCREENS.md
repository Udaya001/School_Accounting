...
Keep the operational navigation to these entries; show only entries a person can access:
...
7. **More** — occasional/compliance area containing **Controls & Audit** (audit/beruju, social and internal control, guarantees, and linked decision evidence).

**Settings** is a separate bottom/utility administration entry, visible only to users with School Administrator configuration permission. It is not part of normal operational navigation.
...
Settings do not contain statutory/accounting rule versions, tax or contribution rules, verified Account Code definitions, or Accounting Engine rules. These are not ordinary school-user settings; their separate system-level configuration boundary is to be designed later.
