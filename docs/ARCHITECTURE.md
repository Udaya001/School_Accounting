...
Use standard locale identifiers. Initial supported locales are `en` and `ne`; the locale registry and translation-key design must allow additional locales later. Resolve the active UI locale in this order: membership or user preference, school default locale, then the system fallback (`en`). Membership preference is scoped to that school membership when present; the global user preference is used when no membership preference is set.

Keep accounting and business data language-neutral: statuses, document types, codes, amounts, accounting dates, and journal semantics are stored as canonical values and never translated values such as Nepali status text. User-entered names and descriptions are stored exactly as entered; transaction fields do not need duplicate English and Nepali copies.

Separate four localization concerns: UI translations cover application labels/messages; reference-data translations provide localized display text for system-controlled records while retaining canonical IDs/codes; user-entered text remains as entered; and official printable template language selects an official template version. English and Nepali print templates can render the same underlying financial data, and `official_template_versions.language` remains part of template identity. Bikram Sambat calendar handling and Nepali-language localization are separate concerns: calendar conversion governs date interpretation, while locale governs language and presentation formatting.
